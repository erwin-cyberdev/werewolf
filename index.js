...
class Bot {
  constructor() {
    ...
    // ── Résolution LID → PN ──────────────────────────────────────────────────
    // WhatsApp (groupes en mode LID) retourne des JIDs @lid au lieu de @s.whatsapp.net.
    // On maintient ici un mapping lid → pn (numéro) pour toujours avoir
    // un JID mentionnable (@s.whatsapp.net) à envoyer à l'API.
    this._lidMap = new Map(); // "xxx@lid"  →  "237xxxxxxx@s.whatsapp.net"
  }

  /**
   * Résout un JID : si c'est un @lid, retourne le @s.whatsapp.net correspondant.
   * Si le mapping n'est pas encore connu, retourne le JID tel quel.
   */
  _resolvePn(jid) {
    if (!jid) return jid;
    if (jid.endsWith("@lid")) return this._lidMap.get(jid) ?? jid;
    return jid;
  }
  ...
  async initialiser() {
    ...
    try {
      ...
      // ── Mapping LID → PN : alimenté automatiquement par Baileys ─────────────
      // Cet event est émis chaque fois que Baileys découvre qu'un @lid
      // correspond à un numéro @s.whatsapp.net (à la réception de messages,
      // lors de la sync des contacts, etc.).
      this.sock.ev.on("lid-mapping.update", ({ lid, pn }) => {
        if (lid && pn) {
          this._lidMap.set(lid, pn);
          console.log(chalk.gray(`[LID] ${lid} → ${pn}`));
        }
      });
  ...
  async traiterMessage(msg) {
    ...
    const sender  = this._resolvePn(getSender(msg)); // résout @lid → @s.whatsapp.net
