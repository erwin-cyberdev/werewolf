...
export default PlayerManager;// ===== Gestion des joueurs : IDs, rôles, statuts, couples, contamination =====
...
class PlayerManager {
  ...
  verifierVictoire() {
    ...
    // Vérification des couples amoureux de camps opposés
    const couplesOpposes = this.joueurs.filter(
      (j) => j.estAmoureux && j.amoureuxId && this.getParId(j.amoureuxId)?.estVivant
    );
    if (couplesOpposes.length === 2) {
      const j1 = couplesOpposes[0];
      const j2 = couplesOpposes[1];
      if (j1.camp !== j2.camp && total === 2) {
        return { equipeGagnante: "couple", gagnants: [j1, j2], raison: "Les amoureux de camps opposés sont les derniers survivants" };
      }
    }
