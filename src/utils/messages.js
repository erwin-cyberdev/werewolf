...
import { formaterListeJoueurs, jidToMention } from "./helpers.js";

// ==================== Messages de groupe ====================

export function msgLancement(pseudo, jid, duree) {
  return `👑 ${jidToMention(jid)} a lancé une partie de Loup-Garou !\n\nLes inscriptions sont ouvertes pour ${duree} secondes.\nTapez \`-join\` pour participer !`;
}

export function msgJoin(pseudo, jid, id) {
  return `✅ ${jidToMention(jid)} a rejoint la partie ! (ID: ${id})`;
}
...
export function msgStartForce(pseudo, jid) {
  return `⚡ ${jidToMention(jid)} a forcé le début de la partie !`;
}
...
export function msgMortNuit(pseudo, jid, role) {
  return `💀 ${jidToMention(jid)} (${role}) a été tué cette nuit.`;
}

// ==================== Causes de mort ====================
// Chaque mort de la partie doit préciser comment le joueur est mort.
// Cette table centralise le libellé (participe passé) associé à chaque cause.
const LIBELLE_CAUSE_MORT = {
  ...
  tonneau: "brûlé vif par le Pyromancien",
  nain_tracassin: "démasqué et tué par le Nain Tracassin",
  chagrin_amoureux: "mort de chagrin après la perte de son/sa partenaire",
  vote: "lynché par le village",
  dictateur: "exécuté sur ordre du Dictateur",
  maire_departage: "lynché sur décision du Maire",
  loup_bavard_puni: "exécuté pour ne pas avoir placé son mot secret",
};

/**
 * Message générique annonçant une mort avec sa cause précise.
 * @param {string} pseudo
 * @param {string} jid
 * @param {string} role
 * @param {string} cause - une des clés de LIBELLE_CAUSE_MORT
 */
export function msgMortCause(pseudo, jid, role, cause) {
  const libelle = LIBELLE_CAUSE_MORT[cause] || "mort";
  return `💀 ${jidToMention(jid)} (${role}) est ${libelle}.`;
}

/**
 * Message annonçant la mort par chagrin d'un amoureux dont le partenaire vient de mourir.
 */
export function msgMortAmoureux(pseudo, jid, role) {
  return `💔 Le cœur brisé, ${jidToMention(jid)} (${role || "Amoureux"}) ${LIBELLE_CAUSE_MORT.chagrin_amoureux} !`;
}
...
export function msgResultatVote(pseudo, jid, nbVotes, totalVotes) {
  return `📊 Résultat du vote : ${jidToMention(jid)} est éliminé avec ${nbVotes}/${totalVotes} voix.`;
}
...
export function msgMaireElu(pseudo, jid) {
  return `👑 ${jidToMention(jid)} est élu Maire du village !`;
}
...
export function msgForceFin(pseudo, jid) {
  return `🛑 ${jidToMention(jid)} a mis fin à la partie.`;
}
...
export function msgAnnonceFossoyeur(pseudo1, jid1, role1, pseudo2, jid2, role2) {
  return `⚰️ Le Fossoyeur désigne ${jidToMention(jid1)} (${role1}).\nDepuis l'au-delà, un nom est apparu : ${jidToMention(jid2)} (${role2}).`;
}

export function msgChasseurTire(pseudoCible, jidCible, ballesRestantes) {
  return `🔫 Le Chasseur a abattu ${jidToMention(jidCible)} !` +
    (typeof ballesRestantes === "number" ? `\n🔫 Balles restantes pour le Chasseur : ${ballesRestantes}/2` : "");
}

export function msgDictateurCoup(pseudoCible, jidCible) {
  return `⚡ Le Dictateur déclenche un coup d'État et impose l'élimination de ${jidToMention(jidCible)} !`;
}

export function msgDictateurMort(roleCible, pseudoDictateur, jidDictateur, pseudoCible, jidCible) {
  if (roleCible === "Loup-Garou") {
    return `👑 ${jidToMention(jidCible)} était un Loup ! ${jidToMention(jidDictateur)} devient Maire du village !`;
  }
  return `💀 ${jidToMention(jidCible)} n'était pas un Loup. ${jidToMention(jidDictateur)} paie de sa vie cette erreur et meurt à son tour.`;
}
...
export function msgCoupleAmoureux(p1, j1, p2, j2) {
  return `💕 ${jidToMention(j1)} et ${jidToMention(j2)} sont amoureux !`;
}

/**
 * Notification privée envoyée à chacun des deux amoureux désignés par Cupidon.
 * @param {string} jidPartenaire - jid de l'autre amoureux
 * @param {string} pseudoPartenaire
 */
export function mpNotificationAmoureux(jidPartenaire, pseudoPartenaire) {
  return `💘 Cupidon a parlé ! Tu es désormais amoureux(se) de ${jidToMention(jidPartenaire)} (${pseudoPartenaire}).\n\n` +
    `❤️ Si l'un de vous deux meurt, l'autre mourra de chagrin aussitôt.\n` +
    `🏆 Si vous êtes les deux derniers survivants de la partie, vous gagnez ensemble !`;
}

export function msgContamination(jp1, jid1, jp2, jid2) {
  return `☣️ ${jidToMention(jid1)} et ${jidToMention(jid2)} ont été contaminés par le Rat Malade.`;
}

export function msgTonnerresExplosion(joueurs) {
  return `💥 Les tonneaux explosent ! ${joueurs.map((j) => jidToMention(j.jid)).join(", ")} ${joueurs.length > 1 ? "sont" : "est"} réduit(s) en cendres.`;
}

export function msgTonnerrePose(joueur) {
  return `🛢️ Un tonneau a été posé chez ${jidToMention(joueur.jid)}.`;
}
...
export function mpGardeInstructions(joueurs, derniereProtection) {
  ...
  if (derniereProtection) {
    msg += `⚠️ Tu as protégé ${jidToMention(derniereProtection.jid)} la nuit dernière.\n\n`;
  }
...
export function mpChasseurInstructions(joueurs, ballesRestantes = 2) {
  return `🔫 C'est la nuit. En tant que Chasseur, tu as ${ballesRestantes} balle${ballesRestantes > 1 ? "s" : ""} dans ton fusil.\n` +
    `Tu peux en utiliser *une seule par nuit* pour abattre le joueur de ton choix.\n\n` +
    `Joueurs disponibles :\n${formaterListeJoueurs(joueurs)}\n\n` +
    `Commandes :\n\`-tirer [id]\`  → ex: -tirer 3 (utilise une balle, action immédiate)\n\`-passer\`      → ne pas tirer cette nuit`;
}
...
export function mpNotificationInfecte() {
  return `🧛 *Tu as été infecté par le Loup Noir !*\n\n` +
    `Tu es désormais un *Loup Noir* toi-même : tu rejoins le camp des Loups et tu agiras ` +
    `chaque nuit avec eux pour désigner une victime à dévorer.\n` +
    `Tes anciens pouvoirs ne sont plus utilisables : tu suivras désormais les instructions des Loups.`;
}

export function mpMercenaireCible(id, jid) {
  return `🎯 Ta cible est ${jidToMention(jid)} (ID: ${id}). Si tu la fais éliminer au vote du jour 1, tu gagnes immédiatement !`;
}
 ...
 */
export function mpFouResultatFaux(id, jidCible, fauxRole) {
  return `🔮 ${jidToMention(jidCible)} (ID : ${id}) est : *${fauxRole}*`;
}
...
export function msgChasseurTire(pseudo, jid) {
  return `🔫 Le Chasseur a tiré sur ${jidToMention(jid)} !`;
}

export function msgDictateurCoup(pseudo, jid) {
  return `⚡ Le Dictateur impose l'élimination de ${jidToMention(jid)} !`;
}
...
export function msgDictateurMort(roleCible, pseudoDictateur, jidDictateur, pseudoCible, jidCible) {
  ...
  return `💀 ${jidToMention(jidCible)} n'était pas un Loup. ${jidToMention(jidDictateur)} paie de sa vie cette erreur.`;
}
