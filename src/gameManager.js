...
import { genererMotBavard, parserCommande, formaterListeJoueurs } from "./utils/helpers.js";
...
class GameManager {
  ...
  async finaliserElectionMaire() {
    ...
    if (resultat.vainqueur) {
      ...
      if (maire) {
        maire.estMaire = true;
        await this.bot.envoyerMessage(this.groupeJid, msg.msgMaireElu(maire.pseudo));
      }
    } else {
  ...
  async finaliserVote(resultat) {
    ...
    if (resultat.vainqueur) {
      ...
      if (elimine) {
        await this.bot.envoyerMessage(this.groupeJid,
          msg.msgResultatVote(elimine.pseudo, resultat.maxVotes, resultat.totalVotes));
        ...
        if (maire && maire.estVivant) {
          ...
          this.actionsEnAttente.push({
            ...
            resolve: async (cibleId) => {
              ...
              await this.bot.envoyerMessage(this.groupeJid,
                `👑 Le Maire (@${maire.pseudo}) a tranché : @${elimine.pseudo} est éliminé !`);
