...
import { jidToMention } from "../utils/helpers.js";
...
for (const loup of loups) {
        await bot.envoyerMessagePrive(loup.jid,
          `👑 L'Héritier (${jidToMention(heritier.jid)}) a hérité des pouvoirs de ${testataire.pseudo} (${testataire.role}) et rejoint notre camp !`);
      }
    } else {
      heritier.role = testataire.role;
      heritier.camp = testataire.camp;
